...
export default function PlayerDetailModal({ player, isOpen, onClose }: PlayerDetailModalProps) {
  ...
  const fetchPlayerDetails = async () => {
    ...
    try {
      ...
      console.log(`🔍 Fetching player details: nfl_id="${player.nfl_id}", season=${player.season}`)
      ...
      console.log('📈 Lifetime EPA:', data.lifetime_epa, 'Lifetime plays:', data.lifetime_plays)
      console.log('🔢 All NGS count:', data.all_ngs?.length)
      setDetailData(data)
    } catch (err: any) {
      console.error('❌ Error fetching player details:', err)
      const errorMessage = err.response?.data?.error || err.message || 'Failed to load player details'
              ...
              {player.status_description && (
                <div className="text-right">
                  <div className={`px-3 py-1 rounded-full text-sm font-semibold ${
                    player.status_description.includes('Injured') ? 'bg-red-500' :
                    player.status_description.includes('Active') ? 'bg-green-500' :
                    'bg-blue-500'
                  }`}>
                    {player.status_description}
                  </div>
          ...
          {!loading && !error && detailData && (
            <div className="border-b bg-gray-50 px-6 py-3">
              <div className="flex items-center justify-between">
                ...
                {/* Season Selector */}
                {viewMode !== 'lifetime' && detailData.all_seasons && detailData.all_seasons.length > 0 && (
                  <div className="flex items-center gap-2">
                    <label htmlFor="season-selector" className="text-sm font-medium text-gray-700">Season:</label>
                    <select
                      id="season-selector"
...
function LifetimeView({ detailData, player }: { detailData: PlayerDetailData; player: Player }) {
  // Aggregate lifetime stats
  const lifetimeStats = detailData.all_stats?.reduce((acc, stat) => {
    return {
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-xl font-bold">Career Summary</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard label="Seasons Played" value={seasonsPlayed.toString()} />
          <MetricCard 
            label="Teams" 
            value={[...new Set(detailData.all_seasons?.map(s => s.team))].join(', ')} 
          />
        </div>
      </div>
      ...
      {detailData.lifetime_plays > 0 && (
        <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xl font-bold">Career Performance Metrics</h3>
          </div>
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        <div className="flex items-center gap-2 mb-4">
          <Activity size={24} />
          <h3 className="text-xl font-bold">Lifetime Statistics</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {player.position === 'QB' && (
            <>
              <StatCard label="Passing Yards" value={lifetimeStats.passing_yards?.toLocaleString() || '0'} />
              <StatCard label="Passing TDs" value={lifetimeStats.passing_tds || 0} />
              <StatCard label="Interceptions" value={lifetimeStats.interceptions || 0} />
              <StatCard label="Rushing Yards" value={lifetimeStats.rushing_yards?.toLocaleString() || '0'} />
            </>
          )}
          ...
          {player.position === 'RB' && (
            <>
              <StatCard label="Rushing Yards" value={lifetimeStats.rushing_yards?.toLocaleString() || '0'} />
              <StatCard label="Rushing TDs" value={lifetimeStats.rushing_tds || 0} />
              <StatCard label="Receptions" value={lifetimeStats.receptions || 0} />
              <StatCard label="Receiving Yards" value={lifetimeStats.receiving_yards?.toLocaleString() || '0'} />
              <StatCard label="Receiving TDs" value={lifetimeStats.receiving_tds || 0} />
            </>
          )}
          ...
          {['LB', 'DE', 'DT', 'CB', 'S', 'ILB', 'OLB', 'MLB', 'NT', 'SS', 'FS', 'DB', 'DL'].includes(player.position) && (
            <>
              <StatCard label="Tackles" value={lifetimeStats.tackles || 0} />
              <StatCard label="Sacks" value={lifetimeStats.sacks?.toFixed(1) || '0'} />
              <StatCard label="Interceptions" value={lifetimeStats.def_interceptions || 0} />
            </>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b">
          <h3 className="text-xl font-bold">Season-by-Season Breakdown</h3>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">EPA</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {detailData.all_seasons?.map((season) => {
                const seasonStats = detailData.all_stats?.find(s => s.season === season.season && s.season_type === 'REGPOST')
                ...
                return (
                  <tr key={season.season} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{season.team}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {seasonStats ? (
                        player.position === 'QB' ? (
                          `${seasonStats.passing_yards?.toLocaleString()} pass yds, ${seasonStats.passing_tds} TDs`
                        ) : player.position === 'RB' ? (
                          `${seasonStats.rushing_yards?.toLocaleString()} rush yds, ${seasonStats.rushing_tds} TDs`
                        ) : (player.position === 'WR' || player.position === 'TE') ? (
                          `${seasonStats.receptions} rec, ${seasonStats.receiving_yards?.toLocaleString()} yds`
                        ) : 'No stats'
                      ) : 'No stats'}
                    </td>
                    <td className="px-4 py-3 text-sm">
                      {seasonEPA && seasonEPA.play_count > 0 ? (
                        <span className={`font-medium ${seasonEPA.epa > 0 ? 'text-green-600' : 'text-red-600'}`}>
...
function SeasonView({ detailData, player, selectedSeason }: { detailData: PlayerDetailData; player: Player; selectedSeason: number }) {
  const seasonData = detailData.all_seasons?.find(s => s.season === selectedSeason) || detailData.player
  const seasonStats = detailData.all_stats?.find(s => s.season === selectedSeason && s.season_type === 'REGPOST')
  const seasonNGS = detailData.all_ngs?.filter(n => n.season === selectedSeason)
    ...
    <>
      ...
      {detailData.epa_by_season && detailData.epa_by_season[selectedSeason] && (
        <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="text-green-600" size={24} />
            <h3 className="text-xl font-bold">{selectedSeason} Performance Metrics</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MetricCard
              ...
              description="Expected Points Added per play"
              color={detailData.epa_by_season[selectedSeason].epa > 0 ? 'green' : 'red'}
            />
            <MetricCard
              label="Total Plays"
              value={detailData.epa_by_season[selectedSeason].play_count.toLocaleString()}
      ...
      {seasonNGS && seasonNGS.length > 0 && (
        <div className="bg-purple-50 rounded-xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xl font-bold">Next Gen Stats</h3>
          </div>
...
function StatSection({ title, icon, player }: { title: string; icon: React.ReactNode; player: Player }) {
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {player.position === 'QB' && (
          <>
            <StatCard label="Passing Yards" value={player.passing_yards?.toLocaleString() || '0'} />
            <StatCard label="Passing TDs" value={player.passing_tds || 0} />
            <StatCard label="Rushing Yards" value={player.rushing_yards?.toLocaleString() || '0'} />
        ...
        {player.position === 'RB' && (
          <>
            ...
            <StatCard label="Receiving Yards" value={player.receiving_yards?.toLocaleString() || '0'} />
        ...
        {isDefensive && (
          <>
            ...
            <StatCard label="Interceptions" value={player.def_interceptions || 0} />
            <StatCard label="Pass Defended" value={player.pass_defended || 0} />
            <StatCard label="Forced Fumbles" value={player.forced_fumbles || 0} />
            <StatCard label="Fumble Recoveries" value={player.fumble_recoveries || 0} />
          </>
        )}
      </div>
    </div>
...
function NextGenStatsDisplay({ ngs, position }: { ngs: any[]; position: string }) {
  if (!ngs || ngs.length === 0) return <p className="text-gray-500">No Next Gen Stats available</p>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      {position === 'QB' && latestNGS.stat_type === 'passing' && (
        <>
          <StatCard label="Avg Time to Throw" value={`${latestNGS.avg_time_to_throw?.toFixed(2)}s` || 'N/A'} />
          <StatCard label="Avg Intended Air Yds" value={`${latestNGS.avg_intended_air_yards?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="Avg Completed Air Yds" value={`${latestNGS.avg_completed_air_yards?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="CPOE" value={`${latestNGS.completion_percentage_above_expectation?.toFixed(1)}%` || 'N/A'} />
          <StatCard label="Max Air Distance" value={`${latestNGS.max_air_distance?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="Air Yards Differential" value={`${latestNGS.avg_air_yards_differential?.toFixed(1)} yds` || 'N/A'} />
        </>
      )}
      
      {position === 'RB' && latestNGS.stat_type === 'rushing' && (
        <>
          <StatCard label="Avg Time to LOS" value={`${latestNGS.avg_time_to_los?.toFixed(2)}s` || 'N/A'} />
          <StatCard label="Rush Yds Over Expected" value={`${latestNGS.rush_yards_over_expected?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="Expected Rush Yards" value={`${latestNGS.expected_rush_yards?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="Efficiency" value={`${(latestNGS.efficiency * 100)?.toFixed(1)}%` || 'N/A'} />
          <StatCard label="Rush vs 8+ Defenders" value={`${(latestNGS.rush_pct_8_defenders * 100)?.toFixed(1)}%` || 'N/A'} />
        </>
      )}
      
      {(position === 'WR' || position === 'TE') && latestNGS.stat_type === 'receiving' && (
        <>
          <StatCard label="Avg Separation" value={`${latestNGS.avg_separation?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="Avg Cushion" value={`${latestNGS.avg_cushion?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="Catch Percentage" value={`${latestNGS.catch_percentage?.toFixed(1)}%` || 'N/A'} />
          <StatCard label="Target Share" value={`${(latestNGS.share_of_team_targets * 100)?.toFixed(1)}%` || 'N/A'} />
          <StatCard label="Avg YAC" value={`${latestNGS.avg_yac?.toFixed(1)} yds` || 'N/A'} />
          <StatCard label="YAC Above Expected" value={`${latestNGS.avg_yac_above_expectation?.toFixed(1)} yds` || 'N/A'} />
        </>
      )}
    </div>
  )
